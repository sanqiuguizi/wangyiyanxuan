...
//	$(".k_areadl a").eq(0).addClass("color");
	...
	var fRank=0;
	var searchStr=$(".k_clcontent").attr("id");
//	
//	//点击切换商品类型
...
function showByKind(searchStr,fRank) {
	$.ajax({
		"async":true,
		"type":"GET",
		"url":"../SearchServlet",
		...
		"success":function(obj){
			$(".k_dataul").html("");
			for(i in obj.searchList){
				//拼接，新品标签,颜色个数标签
				var str1 = "<li id='"+obj.searchList[i].GoodId+"'><p class='k_daimg'><a><img src='"+obj.searchList[i].GoodImg+"'/></a><span>"+obj.searchList[i].GoodColNum+"色可选</span></p><div class='k_datext'>";
				var str2 = "<h4><a>"+obj.searchList[i].GoodName+"</a></h4><p class='k_daprice'>￥"+obj.searchList[i].GoodPrice+"</p><p class='k_daintr'>"+obj.searchList[i].GoodDescribe+"</p></div></li>";
				
				if(obj.searchList[i].GoodColNum==1){
					str1 = "<li id='"+obj.searchList[i].GoodId+"'><p class='k_daimg'><a><img src='"+obj.searchList[i].GoodImg+"'/></a><span style='display: none;'></span></p><div class='k_datext'>";
				}
				
				if(obj.searchList[i].GoodIsNew==1){
